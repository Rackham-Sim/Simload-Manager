...
Source files live in `src/`. Run `build.bat` (or `Ctrl+Shift+B` in VSCode) to compile to LuaJIT bytecode in `dist/` and deploy to your FlyWithLua scripts folder. See `README_BUILD.md` for setup instructions.

Your only place tu work is : C:\Users\Etien\Documents\SITES&PLUGINS\plugins\Simloadmanager\WIP\GITHUB

## Running & Testing

**To run:** Place `SimLoadManager.lua` and the `SLM-Data/` folder in your FlyWithLua scripts directory, then start X-Plane 12. FlyWithLua loads and executes scripts automatically.
...
- **`src/SimLoadManager.lua`** — Main plugin (~2,876 lines). Contains all core logic organized top-to-bottom: initialization, state variables, sound system, settings I/O, SimBrief HTTP integration, embarkation/disembarkation/fuel state machines, ImGui UI, and FlyWithLua event loop registration.
- **`src/SLM-Data/SimLoadManager_loadsheet.lua`** — Secondary floating window module (~436 lines). Renders a detailed loadsheet (planned vs. actual weights, passenger tables, fuel) as a separate ImGui window. It reads from `SLM_Loadsheet_Data`, a global table populated by the main file. Loaded at runtime via `dofile(SCRIPT_DIRECTORY .. "SLM-Data/loadsheet.lua")`.
- **`src/SLM-Data/aircraft.json`** — Aircraft database (ICAO types, weights, pax capacity). Read at runtime by the main plugin.
- **`src/SLM-Data/SimLoad-Manager-Sounds/`** — 16 WAV files used by the sound system.
- **`dist/`** — Compiled LuaJIT bytecode (generated by `build.bat`, not versioned). Do not edit. The upload to GitHub must be done using the files contained in the /dist folder. Mirrors the src/ structure: `dist/SLM-Data/` holds compiled loadsheet and copied assets.
- **`build.bat`** — Build and deploy script. Edit `XPLANE_SCRIPTS` at the top to match your X-Plane installation path.
- **`README_BUILD.md`** — Build setup instructions (LuaJIT install, PATH, configuration).
...
16 WAV files in `src/SLM-Data/SimLoad-Manager-Sounds/`. Loaded via FlyWithLua's `load_WAV_file()`. Volume adjusts dynamically based on whether the camera is internal or external. All sounds are managed through wrapper functions; do not call FlyWithLua sound functions directly in logic code.
