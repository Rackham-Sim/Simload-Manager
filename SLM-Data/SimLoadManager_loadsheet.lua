...
function draw_loadsheet_window(wnd, x, y)
	...
	-- Planned baggage = cargo total - freight (ensures Baggage + Freight = TOTAL)
	local bag_planned = round_kg(cargo_total_planned - freight_planned)

	-- Actual sources (SLM)
	local pax_actual         = tonumber(_G["SLM_real_pax"]) or pax_planned
	local cargo_total_actual = tonumber(_G["SLM_real_cargo"]) or cargo_total_planned

	-- Actual baggage scaled proportionally to loaded pax ratio
	local pax_ratio  = (pax_planned > 0) and (pax_actual / pax_planned) or 1
	local bag_actual = round_kg(bag_planned * pax_ratio)
	...
	local freight_planned_display = (d.slm_source == "manual" or d.slm_source == "fsd") and cargo_total_planned or freight_planned
	imgui.TableNextRow()
...
if d.slm_source ~= "manual" and d.slm_source ~= "fsd" then
	...
	imgui.SameLine(nil, 0)
	imgui.TextUnformatted(slm_load_controller or "SLM")
	imgui.SameLine(nil, 4)
	imgui.PushStyleColor(imgui.constant.Col.Text, 0xFFB0B0B0)
	imgui.TextUnformatted("(Ko-Fi Donator)")
	imgui.PopStyleColor()
