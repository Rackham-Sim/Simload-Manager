        ...
        if (cargo_total or 0) > 0 then
            if slm_rp_last_cargo_unloaded == nil then
                ...
                if delta_units > 0 then
                    ...
                    local per_station = to_kg(delta_units) / #cargo_indices
...
function slm_draw_sequence_steps()
    ...
    if mode == "in_flight" then return end
    ...
    if mode == "turnaround" or mode == "night_stop" then
        ...
        elseif disembark_started then
            ...
            if arr_passengers_total > 0 and passengers_unloaded >= arr_passengers_total then
                slm_draw_step("Passenger Deboarding", "done")
            else
            ...
            if arr_cargo_total > 0 and cargo_unloaded >= arr_cargo_total then
                slm_draw_step("Cargo Unloading", "done")
            else
    ...
    if mode == "turnaround" then
        ...
        if not slm_new_plan_imported then
            imgui.NewLine()
            imgui.PushStyleColor(imgui.constant.Col.Text, 0xFF00A5FF)
            imgui.TextUnformatted("  -- Once flight plan is loaded --")
            imgui.PopStyleColor()
            imgui.NewLine()
            slm_draw_step("Boarding Phase", "pending")
        end
        imgui.NewLine()
    end
    ...
    if mode ~= "turnaround" and mode ~= "night_stop" then
        ...
        if crew_briefing_done then
            slm_draw_step(briefing_label, "done")
        elseif crew_briefing_started then
            ...
            slm_draw_step(briefing_label, "pending")
        end
        imgui.NewLine()
    ...
    local function draw_embark_fuel()
        ...
        elseif embark_started then
            ...
            if slm_defuel_performed then
                ...
                imgui.PushStyleColor(COL.PlotHistogram, 0xFF0080FF)
    ...
    do
        ...
        -- Helper: estimate remaining boarding time (used by turnaround + departure)
        local function boarding_eta()
            if embark_started then
        ...
        -- Helper: estimate remaining pre-boarding time (crew briefing + catering + cleaning[lowcost])
        local function preboard_eta()
            local pax_ref = (passengers_total and passengers_total > 0) and passengers_total or (arr_passengers_total or 0)
            local crew_rem = 0
        ...
        elseif mode == "turnaround" then
            ...
            -- Departure portion
            local t = disembark_remaining + cleaning_arr_rem + preboard_eta() + boarding_eta()
            if t > 0 then total_eta_secs = t end
        ...
        elseif embark_started or crew_briefing_started or catering_started
            ...
            total_eta_secs = preboard_eta() + boarding_eta()
        end

        if total_eta_secs and total_eta_secs > 0 then
            imgui.PushStyleColor(COL.Text, 0xFFFFE080)
            if total_eta_secs < 60 then
                imgui.TextUnformatted("  Total Estimated: < 1 minute")
            else
                local mins = math.ceil(total_eta_secs / 60)
                imgui.TextUnformatted(string.format("  Total Estimated: ~%d minute%s", mins, mins > 1 and "s" or ""))
            end
            imgui.PopStyleColor()
        end
    end
...
if timing_preset == "custom" then
    ...
    imgui.SameLine()
	imgui.TextUnformatted(string.format(
		"-> Realistic: Â±%.1f | Fast: Â±%.1f | Very Fast: Â±%.1f",
...
function build_embark_window(wnd, x, y)
	...
	local load_blocked = (passed_500ft and onground == 0)
		...
		if imgui.Button("Load Simbrief data") then
			if slm_sequence_phase == "waiting_for_new_plan" then
				...
				if last_ofp_timestamp ~= prev_ts then
					...
					slm_auto_import_message = "Same flight plan detected â€” please generate a new OFP on SimBrief."
				end
			else
				fetch_simbrief_data(simbrief_id)
				slm_detect_aircraft()
	...
	imgui.SameLine()
	if not slm_lf_mode then imgui.BeginDisabled() end
	if imgui.Button("Load Last Flight") then
		slm_lf_confirm_open = true
	end
	if not slm_lf_mode then imgui.EndDisabled() end
	...
	if SLM_Loadsheet_Data then
		...
		if slm_data_source == "simbrief" then
			imgui.TextUnformatted("  Please import a new flight or load a saved flight")
		elseif slm_data_source == "manual" then
	...
	if not topup_available and slm_topup_active then slm_topup_active = false end
	...
	if slm_topup_active then
		...
		imgui.Spacing()
		local current_disp = math.floor(from_kg(sim_fuel_total_kg or 0))
		imgui.TextUnformatted(string.format("Current on board : %d %s", current_disp, unit_system))
		imgui.TextUnformatted(string.format("Plan target      : %d %s", fuel_total or 0, unit_system))
		imgui.Spacing()
		local ch, nv = imgui.InputFloat(string.format("New target (%s)##topup", unit_system), slm_topup_new_target, 0, 0, "%.0f")
...
function check_simbrief_trigger()
    if trigger_simbrief_import then
        trigger_simbrief_import = false
        local blocked = (passed_500ft and onground == 0)
            or (passed_500ft and onground == 1 and slm_sequence_mode == nil)
            or (slm_sequence_mode == "departure")
            or (slm_sequence_mode == "turnaround"
                and slm_sequence_phase ~= "waiting_for_new_plan")
